#!/usr/bin/env node
// OpenClaw tool wrapper: get_market_summary
// Spawns market-digest agent.js status --format json with timeout + cooldown
...
const AGENT_NAME = 'market-digest';
const CMD = ['node', path.join(__dirname, '../../src/agents/market-digest/agent.js'), 'status', '--format', 'json'];
const TIMEOUT_MS = 5000;
const COOLDOWN_MS = 60000;
const CACHE_FILE = path.join(__dirname, '.cache-market-summary.json');
...
child.on('close', (code) => {
  ...
  // Only use stdout for JSON (ignore stderr warnings)
