...
class RuntimeInputGenerator {
  ...
  async generate() {
    ...
    const riskOffAnalysis = RiskOffAnalyzer.analyzeRiskOff(marketData, allEventTitles, previousData);

    // 16.7. SECTOR ANALYZER (新增) - 如果有板塊數據
    console.log('📈 分析板塊輪動...');
    let sectorAnalysis = null;
    if (marketData.sectors) {
      const SectorAnalyzer = require('../analyzers/sector-analyzer');
      sectorAnalysis = SectorAnalyzer.analyzeSectorPerformance(marketData.sectors);
    } else {
      // 如果沒有板塊數據，嘗試從新聞推測
      const SectorAnalyzer = require('../analyzers/sector-analyzer');
      const newsSentiment = SectorAnalyzer.inferSectorRotationFromNews(allEventTitles);
      sectorAnalysis = {
        rotation: null,
        newsSentiment,
        note: 'No sector data available, inferred from news'
      };
    }

    // 16.8. RISK-OFF MONITORING (新增)
    ...
    try {
      ...
      if (monitoringResult.alert) {
        console.log(`⚠️  Alert triggered: ${monitoringResult.alert.level} (Score: ${monitoringResult.alert.score})`);
        // TODO: 發送 Telegram 通知
      }
    ...
    const runtimeInput = {
      ...
      risk_off_analysis: riskOffAnalysis,  // 新增：Risk-off 分析
      sector_analysis: sectorAnalysis,      // 新增：板塊分析
