...
- [ ] Console script 匯出群組清單 JSON（含 scroll-to-bottom）
- [ ] `--dry-run` 成功讀取 IMAP、產出 latest.html（兩段式版型）
- [ ] `run` 實際發送 digest email（Top Picks + Everything Else）
- [ ] `--backfill-hours 24` 能補漏且不重複
...
- [ ] `email_parse_ok_rate` >= 90%
- [ ] `post_extract_ok_rate` >= 80%
- [ ] 去重後無大量重複
- [ ] L2 fetch 成功率 >= 30%
- [ ] latest.json 含水位線三件套 + 三段成功率
- [ ] latest.html 排序 deterministic
- [ ] 故障告警正常觸發
