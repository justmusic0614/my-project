...
PATTERN='(botToken"\s*:\s*"[0-9]{6,}:[0-9A-Za-z_-]{20,}")|(xox[baprs]-[0-9A-Za-z-]{10,})|(ghp_[0-9A-Za-z]{30,})|(AKIA[0-9A-Z]{16})|(-----BEGIN (RSA|OPENSSH|EC|DSA) PRIVATE KEY-----)|(sk-[0-9A-Za-z]{20,})'
...
# scan
HITS_RAW="$OUTDIR/02-hits.raw"
HITS_TXT="$OUTDIR/02-hits.txt"
umask 077
: > "$HITS_RAW"
while IFS= read -r f; do
  # grep -nH outputs file:line:match
  grep -nH -E "$PATTERN" "$f" 2>/dev/null >> "$HITS_RAW" || true
done < "$FILES"
...
outdir=sys.argv[1]
hits_path=os.path.join(outdir,"02-hits.raw")
items=[]
...
# produce minimal hits list (file:line only) and remove raw
if [ -f "" ]; then
  awk -F: 'NF>=2{print print("report=",os.path.join(outdir,"report.json"))
PY
":"}' "" | sort -u > ""
  chmod 600 ""
  rm -f ""
...
# --- SRE: finalize outputs (report.json -> 02-hits.txt) ---
REPORT="$OUTDIR/report.json"
HITS_TXT="$OUTDIR/02-hits.txt"
: > "$HITS_TXT"
chmod 600 "$HITS_TXT" || true

if [ -f "$REPORT" ]; then
  python3 - "$REPORT" "$HITS_TXT" <<'PY2'
import json, sys
rp, op = sys.argv[1], sys.argv[2]
try:
...
# ensure raw never persists
rm -f "$OUTDIR/02-hits.raw" 2>/dev/null || true

echo "✅ done: $OUTDIR"

# --- SRE: exit nonzero on hits (for cron/monitoring) ---
HC="$(python3 - <<'PY' "$OUTDIR/report.json"
import json,sys
p=sys.argv[1]
try:
...
if [ "${HC:-0}" -gt 0 ]; then
  echo "🚨 leakscan detected hits: $HC (see $OUTDIR/report.json and 02-hits.txt)"
  exit 2
fi
