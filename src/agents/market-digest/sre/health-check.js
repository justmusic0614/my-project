...
function registerPipelineChecks(healthCheck) {
  ...
  // P6. 指令路由驗證（輕量文件完整性檢查，取代原有 execSync 阻塞驗證）
  healthCheck.register('command-routing', async () => {
    ...
    if (missing.length > 0) {
      throw new Error(`Missing files: ${missing.map(f => path.basename(f)).join(', ')}`);
    }

    const pkg = require('../package.json');
    return { verified: true, version: pkg.version || 'unknown', filesChecked: criticalFiles.length };
  }, { critical: false, timeout: 3000 });
}
