...
// 模組化的 Yahoo Finance 資料源

const DataSourceAdapter = require('../../adapter');

class YahooFinancePlugin extends DataSourceAdapter {
  constructor(config) {
    super('Yahoo Finance', config);
    this.baseUrl = config.baseUrl || 'https://query1.finance.yahoo.com/v8/finance/chart/';
  }
   ...
   */
  async fetch() {
    const symbols = ['^TWII', '^GSPC', 'USDTWD=X'];
    const results = {};
...
module.exports = YahooFinancePlugin;
